...
class HackerNewsNewStoryCommentSpider(scrapy.Spider):
    name = "hacker_news_new_story_comment"
    ...
    def spider_closed(self, spider):
        ...
        for i in sorted_list_of_items:
            ...
            #
            found_item = hn_db.HackerNewsNewStoryComment.query.filter(
                hn_db.HackerNewsNewStoryComment.id == i["id"]
            ).first()
            #
            if found_item:
                hn_db.HackerNewsNewStoryComment.query.filter(
                    hn_db.HackerNewsNewStoryComment.id == i["id"]
                ).update(
                    {
                ...
                i.pop("item_order")
                data = hn_db.HackerNewsNewStoryComment(**i)
    ...
    def start_requests(self):
        found_item = (
            hn_db.HackerNewsNewStory.query.with_entities(hn_db.HackerNewsNewStory.kids)
            .order_by(desc(hn_db.HackerNewsNewStory.parsed_time))
            .limit(500)
        )
    ...
    def item_parse(self, response):
        ...
        if scrape_item:
            ...
            # result_dict["deleted"] = scrape_item.get("deleted", False)
            # result_dict["comment_id"] = scrape_item.get("id")
            ...
            # result_dict["comment_type"] = scrape_item.get("type")
