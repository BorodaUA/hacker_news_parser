...
class HackerNewsNewStoryCommentSpider(scrapy.Spider):
    ...
    def db_connect(self):
        ...
        self.HackerNewsNewStoryComment = self.Base.classes.hacker_news_new_story_comment
        self.Base.session = scoped_session(
            sessionmaker(autocommit=False, autoflush=False, bind=self.engine,)
        )
        self.HackerNewsNewStory.query = self.Base.session.query_property()
        self.HackerNewsNewStoryComment.query = self.Base.session.query_property()
        self.session = Session(self.engine)
        return {
            'HackerNewsNewStory':self.HackerNewsNewStory, 
            'HackerNewsNewStoryComment': self.HackerNewsNewStoryComment,
        ...
        try:
            ...
            for i in sorted_list_of_items:
                ...
                if found_item:
                    ...
                    i.pop("item_order")
                    data = self.HackerNewsNewStoryComment(**i)
